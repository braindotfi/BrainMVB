...
| `server/insightsService.ts` | superseded (cron retired) | `POST /v1/wiki/question` → `GET /api/brain/recommendation` | Daily Anthropic cron over a mock context. **2026-06-25: retired** — its `startDailyInsightsScheduler` call was removed from `server/index.ts` (it also spammed Anthropic 401s at boot), and the HomePage insight line now reads the ledger-grounded `GET /api/brain/recommendation`. The file + the `GET /api/insights` route in `routes.ts` (which imports `generateInsights`/`getInsightsState`) are now unused; delete both in the Phase 8 cleanup. |
