...
`SEED_FILES` (`server/brain/seed.ts`) is the manifest; the bytes live in
`server/assets/demo-seed/` and are (re)generated by `npx tsx scripts/generate-demo-seed.ts`.
Five documents spanning five source categories:

| File | Category | Source type | What it holds |
| --- | --- | --- | --- |
| `bank_statement_2026-06.pdf` | `bank` | `pdf_upload` | operating account, 15 transactions, closes at 165,087.55 |
| `ar_aging_2026-06-30.xlsx` | `accounting` | `csv_upload` | 7 customers outstanding as of 2026-06-30 |
| `payroll_register_2026-06.xlsx` | `payroll` | `csv_upload` | 7 employees, semi-monthly 06-15 / 06-30 |
| `crypto_wallet_2026-06.csv` | `crypto` | `csv_upload` | on-chain treasury wallet, USDC + ETH |
| `form_1120_2025.pdf` | `tax` | `pdf_upload` | FY2025 corporate income tax return |
...
- **Every figure must reconcile with every other figure.** The reconciliation rules are
  documented at the top of `scripts/generate-demo-seed.ts` — read them before adding a
  document. In short: the crypto wallet is on-chain only (the bank statement has no
  crypto transfer line), its customers settled and so are correctly absent from the AR
  aging, and the tax return is prior-year so it cannot contradict June 2026 while still
  deriving its recurring expense lines from the statement's monthly amounts.
