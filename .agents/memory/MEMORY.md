...
- [Replit NODE_ENV is unset outside the workflow process](replit-node-env.md) — use REPLIT_DEV_DOMAIN presence, not NODE_ENV==="development", as the dev-context signal.
...
- [Demo tenant TTL cleanup](demo-tenant-ttl.md) — remote-first deletion is opt-in; persist demo lifecycle separately and claim starts under a DB advisory lock for cross-worker limits.
...
- [Deployment source attestation](deployment-source-attestation.md) — publishing requires a clean workspace synced to origin/main; a feature or memory branch fails before package build.
