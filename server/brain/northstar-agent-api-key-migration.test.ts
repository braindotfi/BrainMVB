...
describe("Northstar-only migration authorization", () => {
  ...
  it("refuses immediately before, exactly at, and after the window", async () => {
    armNorthstar();
    ...
    await expect(migrateAuthorizedNorthstarAgentApiKey()).rejects.toThrow(/outside/);
    vi.setSystemTime(Date.parse(NORTHSTAR_AGENT_API_KEY_MIGRATION_WINDOW_END_ISO) + 1);
    await expect(migrateAuthorizedNorthstarAgentApiKey()).rejects.toThrow(/outside/);
    expect(tenancy.issueBffAgentApiKey).not.toHaveBeenCalled();
  });
  ...
  it("accepts authorization immediately before the exclusive window end", async () => {
    armNorthstar();
    wireNorthstar();
    vi.setSystemTime(Date.parse(NORTHSTAR_AGENT_API_KEY_MIGRATION_WINDOW_END_ISO) - 1);
    await expect(migrateAuthorizedNorthstarAgentApiKey()).resolves.toMatchObject({
