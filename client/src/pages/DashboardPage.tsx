...
const Tag = ({ children, color = "grey" }: { children: React.ReactNode; color?: TagColor }) => {
  ...
  return (
    <span
      ...
      style={{ background: s.bg, border: `1px solid ${s.border}`, color: s.text, fontFamily: "'Gilroy',sans-serif", fontSize: "11px", lineHeight: "14px" }}
    >{children}</span>
  );
};
...
const CategoryTag = ({ children }: { children: React.ReactNode }) => (
  <span
    ...
    style={{ background: "#222737", border: "1px solid rgba(108,119,157,0.2)", color: "#6c779d", fontFamily: "'Gilroy',sans-serif", fontSize: "11px", lineHeight: "14px" }}
  >{children}</span>
);
...
const PanelHeader = ({ title, right }: { title: string; right?: React.ReactNode }) => (
  ...
  >
    <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "16px", lineHeight: "24px", color: "#a8b9f4" }}>{title}</span>
    {right}
  </div>
);
...
const StatCard = ({
  ...
  <div className="flex-1 flex flex-col gap-[8px] p-[16px] rounded-[16px]" style={{ background: CARD_BG }}>
    <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "13px", lineHeight: "14px", color: "#414965" }}>{label}</span>
    <p style={{ fontFamily: "'Gilroy',sans-serif", fontSize: 0, lineHeight: 0, color: "#a8b9f4" }}>
...
const CfBtn = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
  ...
  >
    <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "12px", lineHeight: "16px", color: active ? "#ff9500" : "#414965" }}>{label}</span>
  </button>
);
  ...
  <div className="relative w-full h-full flex flex-col">
    ...
    <div className="relative flex-1">
      ...
      <div className="absolute inset-y-0 right-0 flex flex-col justify-between py-[8px] pr-[8px]" style={{ pointerEvents: "none" }}>
        {CF_Y_LABELS.map((l) => (
          <span key={l} style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "10px", color: "#6c779d", textAlign: "right", lineHeight: "14px" }}>{l}</span>
        ))}
      </div>
    </div>
    ...
    <div className="flex-shrink-0 flex items-center justify-between px-[4px] py-[4px]" style={{ borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
      {xLabels.map((l, i) => (
        <span key={i} style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "10px", color: "#6c779d", padding: "0 4px" }}>{l}</span>
      ))}
    </div>
    ...
    <div className="flex-shrink-0 flex gap-[24px] items-center px-[16px] py-[12px]">
      <div className="flex gap-[8px] items-start">
        ...
        <div className="flex flex-col">
          <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "12px", lineHeight: "14px", color: "#6c779d" }}>Inflow</span>
          <p style={{ fontFamily: "'Gilroy',sans-serif", fontSize: 0, lineHeight: 0, color: "#a8b9f4" }}>
      ...
      <div className="flex gap-[8px] items-start">
        ...
        <div className="flex flex-col">
          <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "12px", lineHeight: "14px", color: "#6c779d" }}>Outflow</span>
          <p style={{ fontFamily: "'Gilroy',sans-serif", fontSize: 0, lineHeight: 0, color: "#a8b9f4" }}>
...
const AccountRow = ({
  ...
  <div className="flex items-start justify-between">
    <div className="flex flex-col gap-[4px] min-w-0 mr-[8px]">
      <span className="truncate" style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "14px", lineHeight: "20px", color: "#a8b9f4" }}>{name}</span>
      <span className="truncate" style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "13px", lineHeight: "16px", color: "#6c779d" }}>{sub}</span>
    </div>
    <div className="flex flex-col items-end gap-[3px] flex-shrink-0">
      <span style={{ fontFamily: "'JetBrains_Mono',sans-serif", fontWeight: 500, fontSize: "14px", lineHeight: "20px", color: "#a8b9f4" }}>{amount}</span>
    ...
    >
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-[12px] px-[16px] py-[12px]">
            ...
            {!walletAcc && !user?.walletAddress && agents.length === 0 && (
              <div className="flex-1 flex items-center justify-center">
                <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "13px", color: "#414965" }}>Connect your wallet to see accounts</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex gap-[16px] flex-shrink-0">
        ...
        <div className="flex-1 rounded-[16px] overflow-hidden flex flex-col" style={{ background: CARD_BG, height: "380px" }}>
          ...
          >
            <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "16px", lineHeight: "24px", color: "#a8b9f4" }}>Recent Activity</span>
          </div>
          <div className="flex-1 overflow-y-auto flex flex-col gap-[12px] px-[16px] py-[12px]">
            {activityItems.map((item, i) => (
              <div key={i} className="flex flex-col gap-[12px]">
                <div className="flex flex-col gap-[4px]">
                  <p style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "14px", lineHeight: "20px", color: "#a8b9f4" }}>
                    {item.spans.map((s, j) => (
                      <span key={j} style={{ color: s.bold ? "#a8b9f4" : "#6c779d" }}>{s.text}</span>
                    ))}
                  </p>
                  <div className="flex items-center gap-[8px]">
                    <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "13px", lineHeight: "16px", color: "#6c779d" }}>{item.time}</span>
                  ...
                  {item.requiresAction && (
                    <div className="flex gap-[8px]">
                      ...
                      >
                        ...
                        <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "12px", lineHeight: "16px", color: "#42bf23" }}>Approve</span>
                      </button>
                      <button
                      ...
                      >
                        ...
                        <span style={{ fontFamily: "'Gilroy',sans-serif", fontSize: "12px", lineHeight: "16px", color: "#d20344" }}>Deny</span>
                      </button>
                    </div>
                  )}
                </div>
