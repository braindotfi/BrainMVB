 ...
 *
 * Width variants (pass via `widthClass`):
 *   standard  w-[480px]  — detail popups (account, bill, transaction, vendor, audit, proposal…)
 *   form      w-[400px]  — single-purpose form dialogs (contact update, billing, security)
 *   compact   w-[375px]  — confirmation dialogs (DeleteConfirmDialog)
 *
 * All variants share the same overlay (bg-black/60 backdrop-blur-[2px]), background
 * (bg-brain-v1baby-blue-5 / #11141b), border (brain-v1stroke-2 / #1d2132), radius
 * (rounded-modal / 24px), and shadow (0 24px 60px rgba(0,0,0,0.6)).
 */
export function DetailPopupShell({
  ...
  onClose,
  closeTestId,
  widthClass = "w-[480px] max-w-[calc(100vw-32px)]",
  children,
}: {
  title: string;
  open: boolean;
  onClose: () => void;
  closeTestId?: string;
  /** Tailwind width classes. Defaults to "w-[480px] max-w-[calc(100vw-32px)]" (standard).
   *  Use "w-[400px] max-w-[calc(100vw-32px)]" for form dialogs,
   *  "w-[375px] max-w-[calc(100vw-32px)]" for compact confirm dialogs. */
  widthClass?: string;
  children: ReactNode;
}) {
  return (
